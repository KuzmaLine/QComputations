...
class BLOCKED_Vector: public BLOCKED_Matrix<T> {
    public:
        ...
        explicit BLOCKED_Vector(ILP_TYPE ctxt, const std::vector<T>& x): BLOCKED_Matrix<T>(ctxt, GE, Matrix<T>(FORTRAN_STYLE, x, x.size(), 1)) {};
        ...
        std::vector<T> get_local_vector() const { return this->get_local_matrix().get_mass();}
        std::vector<T>& get_local_vector() { return this->get_local_matrix().get_mass();}
...
BLOCKED_Vector<COMPLEX> operator*(const BLOCKED_Matrix<COMPLEX>& A, const BLOCKED_Vector<COMPLEX>& x);
