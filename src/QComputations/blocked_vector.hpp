...
template <typename T> class BLOCKED_Vector : public BLOCKED_Matrix<T> {
  public:
...
BLOCKED_Vector<T>::BLOCKED_Vector(const BLOCKED_Vector<T> &A, const Matrix<T> &x)
    : BLOCKED_Matrix<T>::ctxt_(A.ctxt()), BLOCKED_Matrix<T>::matrix_type_(A.get_matrix_type()),
      BLOCKED_Matrix<T>::n_(A.n()), BLOCKED_Matrix<T>::m_(A.m_), BLOCKED_Matrix<T>::NB_(A.NB()),
      BLOCKED_Matrix<T>::MB_(A.MB_), BLOCKED_Matrix<T>::local_matrix_(x) {}

template <typename T> BLOCKED_Vector<T> BLOCKED_Vector<T>::operator+(T num) const {
    BLOCKED_Vector<T> res(*this, BLOCKED_Matrix<T>::local_matrix_ + num);

    return res;
}

template <typename T> std::vector<T> BLOCKED_Vector<T>::get_vector() const {
