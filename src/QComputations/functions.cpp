...
std::vector<COMPLEX> fftshift(const std::vector<COMPLEX>& in) {
    size_t N = in.size();
...
std::vector<COMPLEX> fft_forward(const std::vector<COMPLEX>& v) {
    // auto in = v;
    // fftw_plan plan = fftw_plan_dft_1d(v.size(),
    ...
    // double norm = 1.0 / std::sqrt(static_cast<double>(v.size()));
    // for (size_t i = 0; i < v.size(); ++i) in[i] *= norm;
    ...
    if (status != DFTI_NO_ERROR) throw std::runtime_error("DftiCreateDescriptor error");
    ...
    status = DftiComputeForward(handle, in.data(), out.data());
...
std::vector<COMPLEX> fft_backward(const std::vector<COMPLEX>& v) {
    ...
    status = DftiComputeBackward(handle, in.data(), out.data());
    DftiFreeDescriptor(&handle);
