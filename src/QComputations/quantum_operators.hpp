...
namespace {
template <typename T> void _printName(std::function<State<T>(const T &)> func, const std::string &funcName) {
...
template <typename StateType> using OperatorT = std::function<State<StateType>(const StateType &state)>;
...
template <typename StateType> class Operator {
  ...
  public:
    ...
    Operator<StateType> operator*(Operator<StateType> other) {
        if (other.root_ == NULL)
            return (*this);
        if (this->root_ == NULL)
            assert(false); // Пустой оператор умножить на не пустой?
    ...
    Operator<StateType> operator*(const COMPLEX &num) {
        if (this->root_ == NULL)
            assert(false); // Пустой оператор умножить на число?
...
template <typename StateType> void Operator<StateType>::refresh_tree() const {
    std::stack<OperatorNode *> st;
    OperatorNode *cur_op = this->root_;
...
template <typename StateType> void Operator<StateType>::show() const {
    std::stack<OperatorNode *> st;
    OperatorNode *cur_op = this->root_;
...
template <typename StateType> State<StateType> Operator<StateType>::run(const State<StateType> &init_state) const {
    OperatorNode *cur_op = this->root_;
...
template <typename StateType>
BLOCKED_Matrix<COMPLEX> operator_to_matrix(ILP_TYPE ctxt, const Operator<StateType> &op,
...
template <typename StateType>
State<StateType> set_qudit(const StateType &state, ValType val, size_t qudit_index = 0, size_t group_id = 0,
...
template <typename StateType>
State<StateType> get_qudit(const StateType &state, size_t qudit_index = 0, size_t group_id = 0,
                           const std::string &info = "") {
...
template <typename StateType>
State<StateType> sigma_x(const StateType &state, size_t qudit_index = 0, size_t group_id = 0,
                         const std::string &info = "") {
    StateType res = state;
...
template <typename StateType>
State<StateType> sigma_y(const StateType &state, size_t qudit_index = 0, size_t group_id = 0,
                         const std::string &info = "") {
    StateType res = state;
    auto qudit = state.get_qudit(qudit_index, group_id);
    assert(qudit == 0 or qudit == 1);
...
template <typename StateType>
State<StateType> sigma_z(const StateType &state, size_t qudit_index = 0, size_t group_id = 0,
                         const std::string &info = "") {
...
template <typename StateType>
State<StateType> check(const StateType &state, ValType check_val, size_t qudit_index = 0, size_t group_id = 0,
                       const std::string &info = "") {
    auto res = state;
