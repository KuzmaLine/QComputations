...
namespace QComputations {
    ...
    std::vector<double> mpi::scatter_blacs_vector<double>(const std::vector<double>& v,
        ...
        for (ILP_TYPE r = 0; r < N; r += NB, sendr = (sendr + 1) % proc_rows) {
            ...
            if (myrow == sendr && mycol == sendc) {
                MPI_Recv(
                    local_v.data() + recvr, nr, MPI_DOUBLE, root_id, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
    ...
    void mpi::gather_blacs_vector<COMPLEX>(const std::vector<COMPLEX>& local_y,
        ...
        for (ILP_TYPE r = 0; r < N; r += NB, sendr = (sendr + 1) % proc_rows) {
            ...
            if (rank == root_id) {
                ILP_TYPE source_id = blacs_pnum(&ctxt, &sendr, &iZERO);
                MPI_Recv(
                    y.data() + r, nr, MPI_DOUBLE_COMPLEX, source_id, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
    ...
    template <>
    std::vector<COMPLEX> mpi::get_diagonal_elements<COMPLEX>(Matrix<COMPLEX>& localA,
