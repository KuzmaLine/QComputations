...
std::vector<COMPLEX> fftshift(const std::vector<COMPLEX>& v);
std::vector<COMPLEX> ifftshift(const std::vector<COMPLEX>& v);
std::vector<COMPLEX> fft_forward(const std::vector<COMPLEX>& v);
std::vector<COMPLEX> fft_backward(const std::vector<COMPLEX>& v);
...
template<typename StateType>
void show_basis(const std::vector<std::shared_ptr<StateType>>& basis) {
    for (const auto& state: basis) {
        std::cout << std::setw(QConfig::instance().width()) << state->to_string() << " ";
    }
...
double scalar_product(double* a, double* b, ILP_TYPE size); 
COMPLEX scalar_product(COMPLEX* a, COMPLEX* b, ILP_TYPE size);
int scalar_product(const std::vector<int>& a, const std::vector<int>& b);
#endif

int scalar_product(int* a, int* b, ILP_TYPE size);

int and_sum(int* a, int* b, ILP_TYPE size);
...
template<typename T>
T get_state_from_basis(const std::unordered_map<T, size_t>& basis, size_t index) {
    for (auto& p: basis) {
        if (p.second == index) {
            return p.first;
        }
...
template<typename StateType>
BasisType<StateType> make_full_basis(const std::shared_ptr<StateType>& state) {
    BasisType<StateType> basis;
    auto cur_state = *state;
    cur_state.set_zero();
    basis.insert(std::make_shared<StateType>(cur_state));

    for (int i = 0; i < cur_state.qudits_count(); i++) {
...
template<typename StateType>
std::vector<std::shared_ptr<Basis_State>> convert_to(const std::vector<std::shared_ptr<StateType>>& states) {
    std::vector<std::shared_ptr<Basis_State>> res;
    std::copy(states.begin(), states.end(), std::back_inserter(res));
    return res;
}

template<typename StateType>
std::vector<std::string> basis_to_string(const BasisType<StateType>& states) {
    std::vector<std::string> res;
    for (auto st: states) {
        res.emplace_back(st->to_string());
    }
