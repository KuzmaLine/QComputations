...
namespace {
    ...
    ILP_TYPE get_index_value(ILP_TYPE row_index, const MKL_INT* ia) {
        return ia[row_index];
    }
}
...
class CSR_Matrix {
    public:
        ...
        explicit CSR_Matrix(ILP_TYPE n, ILP_TYPE m, const std::vector<T>& vals, const std::vector<MKL_INT>& ia, const std::vector<MKL_INT>& ja);
        explicit CSR_Matrix(const std::function<void(std::vector<MKL_INT>& ia, std::vector<MKL_INT>& ja, std::vector<T>& vals)>&, T default_value = T(0));
...
template<typename T>
CSR_Matrix<T>::CSR_Matrix(const Matrix<T>& A, T default_value) : n_(A.n()), m_(A.m()), default_value_(default_value) {
...
template<typename T>
CSR_Matrix<T>::CSR_Matrix(ILP_TYPE n, ILP_TYPE m, const std::vector<T>& vals, const std::vector<MKL_INT>& ia, const std::vector<MKL_INT>& ja): n_(n), m_(m) {
    ia_ = std::make_unique<MKL_INT[]>(n_ + 1);
