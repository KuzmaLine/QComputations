...
namespace {
    ...
    void to_mkl_sparse(ILP_TYPE n, ILP_TYPE m, MKL_INT* ia, MKL_INT* ja, COMPLEX* data, sparse_matrix_t* A) {
        mkl_sparse_z_create_csr(A, SPARSE_INDEX_BASE_ZERO, n, m, ia, ia + 1, ja, reinterpret_cast<MKL_Complex16*>(data));
    }
...
template<>
CSR_Matrix<double>::CSR_Matrix(ILP_TYPE n, ILP_TYPE m, const std::vector<double>& vals, const std::vector<MKL_INT>& ia, const std::vector<MKL_INT>& ja): n_(n), m_(m) {
...
template<>
CSR_Matrix<COMPLEX>::CSR_Matrix(ILP_TYPE n, ILP_TYPE m, const std::vector<COMPLEX>& vals, const std::vector<MKL_INT>& ia, const std::vector<MKL_INT>& ja): n_(n), m_(m) {
    ia_ = std::make_unique<MKL_INT[]>(n_ + 1);
    std::copy(ia.begin(), ia.end(), ia_.get());
...
template<>
CSR_Matrix<double>::CSR_Matrix(const Matrix<double>& A, double default_value) : n_(A.n()), m_(A.m()), default_value_(default_value) {
...
template<>
CSR_Matrix<COMPLEX>::CSR_Matrix(const Matrix<COMPLEX>& A, COMPLEX default_value) : n_(A.n()), m_(A.m()), default_value_(default_value) {
    std::vector<MKL_INT> ia_vec;
    std::vector<MKL_INT> ja_vec;
...
void optimized_multiply(const CSR_Matrix<double>& A, const Matrix<double>& B, Matrix<double>& C, double alpha, double betta, char op) {
    ...
    mkl_sparse_d_mm(tmp, alpha, A.mkl_matrix(), descr, layout, const_cast<double*>(B.data()), B.m(), B.LD(), betta, C.data(), C.LD());
}
...
void optimized_multiply(const CSR_Matrix<COMPLEX>& A, const Matrix<COMPLEX>& B, Matrix<COMPLEX>& C, COMPLEX alpha, COMPLEX betta, char op) {
    ...
    mkl_sparse_z_mm(tmp, mkl_alpha, A.mkl_matrix(), descr, layout, reinterpret_cast<MKL_Complex16*>(const_cast<COMPLEX*>(B.data())), B.m(), B.LD(), mkl_betta, reinterpret_cast<MKL_Complex16*>(C.data()), C.LD());
}
