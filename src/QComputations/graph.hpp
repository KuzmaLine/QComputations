...
template <typename StateType> bool is_in_basis(const BasisType<StateType> &basis, std::shared_ptr<StateType> state) {
    for (auto st : basis) {
        if ((*st) == (*state)) {
            return true;
        }
...
template <typename StateType> class State_Graph {
  public:
    explicit State_Graph(const State<StateType> &init_state, const Operator<StateType> &A_op,
...
State_Graph<StateType>::State_Graph(const State<StateType> &init_state, const Operator<StateType> &A_op,
                                    const std::vector<Operator<StateType>> &operator_decoherence) {
