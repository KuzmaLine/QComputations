...
template <typename T> class BLOCKED_Matrix {
  public:
...
template <typename T> BLOCKED_Matrix<T> BLOCKED_Matrix<T>::operator/(T num) const {
...
template <typename T> void BLOCKED_Matrix<T>::print_distributed(const std::string &name) const {
    ILP_TYPE rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (matrix_type_ == HE) {
        if (rank == mpi::ROOT_ID)
            std::cout << "Hermit Matrix!" << std::endl;
    }
...
template <typename T> ILP_TYPE BLOCKED_Matrix<T>::get_global_row(size_t i) const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> ILP_TYPE BLOCKED_Matrix<T>::get_global_col(size_t j) const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> ILP_TYPE BLOCKED_Matrix<T>::get_row_proc(size_t i) const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> ILP_TYPE BLOCKED_Matrix<T>::get_col_proc(size_t j) const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> ILP_TYPE BLOCKED_Matrix<T>::get_local_row(size_t i) const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> ILP_TYPE BLOCKED_Matrix<T>::get_local_col(size_t j) const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> bool BLOCKED_Matrix<T>::is_my_elem_row(size_t i) const {
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
template <typename T> bool BLOCKED_Matrix<T>::is_my_elem_col(size_t j) const {
    ILP_TYPE proc_rows, proc_cols, myrow, mycol;
    mpi::blacs_gridinfo(ctxt_, proc_rows, proc_cols, myrow, mycol);
...
BLOCKED_Matrix<T>::BLOCKED_Matrix(const BLOCKED_Matrix<T> &A, const Matrix<T> &local_matrix)
    : matrix_type_(A.matrix_type_), ctxt_(A.ctxt_), n_(A.n_), m_(A.m_), NB_(A.NB_), MB_(A.MB_),
...
template <typename T> void BLOCKED_Matrix<T>::operator+=(T num) { local_matrix_ += num; }
...
template <typename T> void BLOCKED_Matrix<T>::operator+=(const BLOCKED_Matrix<T> &B) { *this = *this + B; }
...
template <typename T>
BLOCKED_Matrix<T>::BLOCKED_Matrix(ILP_TYPE ctxt, MATRIX_TYPE type, size_t n, size_t m,
                                  std::function<T(size_t, size_t)> func, size_t NB, size_t MB)
...
template <typename T> std::vector<ILP_TYPE> BLOCKED_Matrix<T>::desc() const {
    ILP_TYPE iZERO = 0;
    ILP_TYPE info;
...
template <typename T> void BLOCKED_Matrix<T>::show(size_t width, ILP_TYPE root_id) const {
    ILP_TYPE rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
...
void optimized_add(const BLOCKED_Matrix<COMPLEX> &A, BLOCKED_Matrix<COMPLEX> &C, COMPLEX alpha, COMPLEX betta,
                   char trans_A);
template <typename T>
void optimized_multiply(const BLOCKED_Matrix<T> &A, const BLOCKED_Matrix<T> &B, BLOCKED_Matrix<T> &C, T alpha, T betta,
...
// SPECIAL FOR QME
std::vector<BLOCKED_Matrix<COMPLEX>>
MPI_Runge_Kutt_2(const std::vector<double> &x, const BLOCKED_Matrix<COMPLEX> &y0,
                 std::function<void(double, const BLOCKED_Matrix<COMPLEX> &, BLOCKED_Matrix<COMPLEX> &)> f);

std::vector<BLOCKED_Matrix<COMPLEX>>
MPI_Runge_Kutt_4(const std::vector<double> &x, const BLOCKED_Matrix<COMPLEX> &y0,
                 std::function<void(double, const BLOCKED_Matrix<COMPLEX> &, BLOCKED_Matrix<COMPLEX> &)> f);
