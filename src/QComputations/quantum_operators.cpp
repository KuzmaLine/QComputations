...
State<X_State> X_OP(const std::shared_ptr<X_State>& state) {
    return State<X_State>(state) * state->get_x();
}

State<P_State> P_OP(const std::shared_ptr<P_State>& state) {
    return State<P_State>(state) * state->get_p();
}

State<TCH_State> photons_transfer(const std::shared_ptr<TCH_State>& st) {
    State<TCH_State> res;
    ...
    for (size_t i = 0; i < state.cavities_count(); i++) {
        ...
        for (size_t w_from = 0; w_from < state.freqs_count(); w_from++) {
            for (size_t w_to = w_from + 1; w_to < state.freqs_count(); w_to++) {
                for (auto cavity_id: neighbours) {
                    if (state.n(i, w_from, w_to) != 0) {
                        state.set_n(state.n(i, w_from, w_to) - 1, i, w_from, w_to);
                        state.set_n(state.n(cavity_id, w_from, w_to) + 1, cavity_id, w_from, w_to);

                        res += State<TCH_State>(state) 
                                * state.get_gamma(i, cavity_id) * std::sqrt(state.n(i, w_from, w_to) + 1)
                                * std::sqrt(state.n(cavity_id, w_from, w_to));
...
State<TCH_State> exc_relax_atoms(const std::shared_ptr<TCH_State>& st) {
    State<TCH_State> res;
    ...
    for (size_t i = 0; i < state.cavities_count(); i++) {
        for (size_t w_from = 0; w_from < state.freqs_count(); w_from++) {
            for (size_t w_to = w_from + 1; w_to < state.freqs_count(); w_to++) {
                if (state.n(i, w_from, w_to) != 0) {
                    state.set_n(state.n(i, w_from, w_to) - 1, i, w_from, w_to);
                    for (size_t j = 0; j < state.m(i); j++) {
                        if (state.get_atom(j, i) == w_from and state.is_move(j, i, w_from, w_to)) {
                            res += set_qudit(std::make_shared<TCH_State>(state), w_to, state.ph_types_count() + j, i) * state.g(j, i, w_from, w_to) * std::sqrt(state.n(i, w_from, w_to) + 1);
                        }
                    }
                    state.set_n(state.n(i, w_from, w_to) + 1, i, w_from, w_to);
                }
                ...
                for (size_t j = 0; j < state.m(i); j++) {
                    //std::cout << j << " " << i << " " << w_from << " " << w_to << " - " << state.get_atom(j, i) << std::endl;
                    if (state.get_atom(j, i) == w_to and state.is_move(j, i, w_from, w_to) and state.n(i, w_from, w_to) != QConfig::instance().max_photons()) {
                        state.set_atom(w_from, j, i);
                        state.set_n(state.n(i, w_from, w_to) + 1, i, w_from, w_to);
                        res += State<TCH_State>(state) * state.g(j, i, w_from, w_to) * std::sqrt(state.n(i, w_from, w_to));
                        state.set_atom(w_to, j, i);
                        state.set_n(state.n(i, w_from, w_to) - 1, i, w_from, w_to);
                    }
                }
            }
...
State<TCH_State> photons_count(const std::shared_ptr<TCH_State>& state) {
    ...
    for (size_t i = 0; i < state->cavities_count(); i++) {
        ...
        //std::cout << state.get_qudit(0, i) << std::endl;
                res[0] += state->n(i, w_from, w_to) * QConfig::instance().h() * state->w_move(w_from, w_to);
            }
        }
    }
...
State<TCH_State> atoms_exc_count(const std::shared_ptr<TCH_State>& state) {
    ...
    for (size_t i = 0; i < state->cavities_count(); i++) {
        for (size_t j = 0; j < state->m(i); j++) {
            res[0] += QConfig::instance().h() * state->w(state->get_atom(j, i));
        }
    }
