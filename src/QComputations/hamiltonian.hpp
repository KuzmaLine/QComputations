...
template <typename StateType> class H_by_Operator : public Hamiltonian {
  public:
...
template <typename StateType> class H_by_Scalar_Product : public Hamiltonian {
  public:
...
H_by_Scalar_Product<StateType>::H_by_Scalar_Product(
    const State<StateType> &init_state, const std::function<COMPLEX(const StateType &i, const StateType &j)> &func,
    BasisType<StateType> basis) {
    if (basis.empty()) {
