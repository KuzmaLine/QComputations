...
// #endif
    ...
    protected:
        ...
        CUDA_CSR_Matrix<COMPLEX> H_;
        // CSR_Matrix<COMPLEX> eigenvectors_;
        // std::vector<double> eigenvalues_;
        std::vector<std::pair<double, CUDA_CSR_Matrix<COMPLEX>>> decoherence_;
        // CSR_Matrix<COMPLEX> H_EXP_;
        // double dt_exp_ = 0;
};

template<typename StateType>
class CUDA_CSR_H_by_Operator: public CUDA_CSR_Hamiltonian {
    public:
        explicit CUDA_CSR_H_by_Operator(cusparseHandle_t handle, const State<StateType>& init_state, const Operator<StateType>& H_op,
...
class CUDA_CSR_H_TCH : public CUDA_CSR_H_by_Operator<TCH_State> {
    public:
        explicit CUDA_CSR_H_TCH(cusparseHandle_t handle, const State<TCH_State>& state);
};
