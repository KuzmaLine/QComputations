...
template <> BLOCKED_Vector<COMPLEX> BLOCKED_Vector<COMPLEX>::operator*(COMPLEX num) const {
...
template <> BLOCKED_Vector<double> BLOCKED_Vector<double>::operator*(double num) const {
...
template <> BLOCKED_Vector<double> BLOCKED_Vector<double>::operator+(const BLOCKED_Vector<double> &x) const {
    BLOCKED_Vector<double> res(*this);
...
template <> void BLOCKED_Vector<double>::operator+=(const BLOCKED_Vector<double> &x) {
    mpi::parallel_daxpy(x.get_local_matrix().get_mass(), this->get_local_matrix().get_mass(), x.desc(), x.inc(),
                        this->desc(), this->inc(), 1.0);
}

template <> BLOCKED_Vector<COMPLEX> BLOCKED_Vector<COMPLEX>::operator+(const BLOCKED_Vector<COMPLEX> &x) const {
    BLOCKED_Vector<COMPLEX> res(*this);
...
template <> void BLOCKED_Vector<COMPLEX>::operator+=(const BLOCKED_Vector<COMPLEX> &x) {
