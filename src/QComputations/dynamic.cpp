...
BLOCKED_Probs schrodinger(const State<Basis_State> &init_state, BLOCKED_Hamiltonian &H,
    ...
    std::vector<COMPLEX> lambda;
    for (size_t i = 0; i < eigen_values.size(); i++) {
        lambda.emplace_back(scalar_product(vector_of_eigen_vectors[i],
                                           blocked_init_state)); // <PHI_i|KSI(0)>
    }
...
Probs quantum_master_equation(const State<Basis_State> &init_state, Hamiltonian &H, const std::vector<double> &time_vec,
    ...
    for (const auto &p : H.get_decoherence()) {
        ...
        lindblads.push_back(std::function<void(const Rho &rho)>{[A, &T1, &T2, gamma](const Rho &rho) {
            optimized_multiply(A, A, T1, COMPLEX(1, 0), COMPLEX(0, 0), 'C',
                               'N');                                       // AconjA -> T1
            ...
            optimized_multiply(T1, A, T2, COMPLEX(gamma, 0), COMPLEX(-0.5 * gamma, 0), 'N',
                               'C'); // res -> T2
        }});
    }
...
BLOCKED_Probs quantum_master_equation(const std::vector<COMPLEX> &init_state, BLOCKED_Hamiltonian &H,
    ...
    for (size_t cavity_id = 0; cavity_id < grid.cavities_count(); cavity_id++) {
        ...
        if (!is_zero(gamma)) {
            ...
            lindblads.push_back(std::function<BLOCKED_Rho(const BLOCKED_Rho &rho)>{[A, gamma](const BLOCKED_Rho &rho) {
                auto Aconj = A.hermit();
                auto AconjA = Aconj * A;
