...
Matrix<COMPLEX> create_A_destroy(const std::set<Basis_State>& basis, size_t cavity_id) {
    ...
    Matrix<COMPLEX> A(C_STYLE, dim, dim, COMPLEX(0));
...
Probs exp_evolution(const State<Basis_State>& init_state, Hamiltonian& H, double dt, size_t STEPS_COUNT, const std::function<void(std::vector<COMPLEX>&)>& func) {
    auto state = init_state.fit_to_basis_state(H.get_basis()).get_vector();
    H.find_exp(dt);
...
Probs schrodinger(const State<Basis_State>& init_state, Hamiltonian& H, const std::vector<double>& time_vec) {
    ...
    for (size_t i = 0; i < eigen_values.size(); i++) {
        ...
        lambda.emplace_back(eigen_vectors.col(i) | init_state_vec); // <PHI_i|KSI(0)> 
    }
    ...
    for (size_t t_index = 0; t_index < time_vec.size(); t_index++) {
        double t = time_vec[t_index];
...
BLOCKED_Probs schrodinger(const State<Basis_State>& init_state, BLOCKED_Hamiltonian& H,
    ...
    auto init_state_vec = init_state.fit_to_basis_state(H.get_basis()).get_vector();
    BLOCKED_Vector<COMPLEX> blocked_init_state(vector_ctxt, init_state_vec);
    ...
    for (const auto& t: time_vec) {
        ...
        for (size_t i = 0; i < eigen_values.size(); i++) {
            ...
            mpi::parallel_zaxpy(vector_of_eigen_vectors[i].get_local_matrix().get_mass(),
                                psi_t.get_local_matrix().get_mass(), vector_of_eigen_vectors[i].desc(),
                                vector_of_eigen_vectors[i].inc(),
                                psi_t.desc(), psi_t.inc(),
                                lambda[i] * std::exp(COMPLEX(0, -1 / h * eigen_values[i] * t)));
        }
...
std::pair<Probs, std::vector<std::shared_ptr<Basis_State>>> probs_to_group_probs(const Probs& probs,
    ...
    State<Basis_State> basis_res;
    auto states = sort_basis(basis);
    for (auto cur_state: basis) {
        basis_res.insert(cur_state->get_group(group_id));
    }
    ...
    for (size_t t = 0; t < probs.m(); t++) {
        for (size_t i = 0; i < probs.n(); i++) {
            size_t res_index = basis_res.get_index(states[i]->get_group(group_id));
            res[res_index][t] += probs.elem(i, t);
        }
    }

    return std::make_pair(res, basis_res.get_basis());
}

std::pair<Probs, std::vector<std::shared_ptr<Basis_State>>> probs_to_qudits(const Probs& probs,
                                                          const BasisType<Basis_State>& basis, const std::vector<size_t>& qudits) {
    State<Basis_State> state_res;
    auto states = sort_basis(basis);
    for (auto cur_state: states) {
        state_res.insert(cur_state->reduce(qudits));
    }

    state_res.sort();

    size_t m = probs.m();
    Probs res(C_STYLE, state_res.size(), m, double(0));

    // #pragma omp parallel for
    for (size_t t = 0; t < probs.m(); t++) {
        for (size_t i = 0; i < probs.n(); i++) {
            size_t res_index = state_res.get_index(states[i]->reduce(qudits));
            res[res_index][t] += probs.elem(i, t);
        }
    }

    return std::make_pair(res, state_res.get_basis());
}
...
// Переделать на шаблоны
 std::pair<BLOCKED_Probs, std::vector<std::shared_ptr<Basis_State>>> probs_to_cavity_probs(const BLOCKED_Probs& probs,
                                                          const std::vector<std::shared_ptr<Basis_State>>& basis, size_t cavity_id) {
    ...
    for (auto cur_state: basis) {
        size_t res_index = basis_res.get_index(cur_state->get_group(cavity_id));
    ...
    for (size_t i_local = 0; i_local < probs.local_n(); i_local++) {            
        ...
        size_t res_index = basis_res.get_index(basis[i]->get_group(cavity_id));
...
// BLOCKED_Matrix<COMPLEX> create_BLOCKED_A_destroy(ILP_TYPE ctxt, const std::vector<std::shared_ptr<Basis_State>>& basis, size_t cavity_id) {
//     size_t dim = basis.size();
...
BLOCKED_Probs quantum_master_equation(const State<Basis_State>& init_state,
    ...
    // BLOCKED_Matrix<COMPLEX> rho_0(create_BLOCKED_init_rho(H.ctxt(), init_state.fit_to_basis_state(H.get_basis()).get_vector()));
    ...
    // auto end_c = std::chrono::steady_clock::now();
    // std::cout << " BLOCKED_QME_TIME " << std::chrono::duration_cast<std::chrono::milliseconds>(end_c - begin_c).count() << std::endl;
    // //std::cout << "HERE 2\n";
    ...
    // }
    // */
    return quantum_master_equation(init_state.fit_to_basis_state(H.get_basis()).get_vector(), H, time_vec, is_full_rho);
}
...
BLOCKED_Probs quantum_master_equation(const std::vector<COMPLEX>& init_state,
    ...
    */
    ...
    BLOCKED_Matrix<COMPLEX> rho_0(create_BLOCKED_init_rho(H.ctxt(), init_state));
    //std::cout << "RHO_0: " << rho_0.matrix_type() << std::endl;
    //rho_0.show();
    auto begin_c = std::chrono::steady_clock::now();
    //std::cout << "HERE\n";
...
std::vector<double> blocked_scan_gamma(const TCH_State& init_state,
    ...
    for (auto state: basis) {
        ...
        for (size_t i = 0; i < state->cavities_count(); i++) {
            ...
            zero_state_in_basis = state->is_all_qudits_zero();
        }
