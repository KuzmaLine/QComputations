...
template<>
Matrix<double>& Matrix<double>::operator+=(const Matrix<double>& other) {
    ILP_TYPE iONE = 1;
    ILP_TYPE size = this->n() * this->m();
    double one = 1;

    daxpy(&size, &one, other.data(), &iONE, this->data(), &iONE);

    return *this;
}

template<>
Matrix<COMPLEX>& Matrix<COMPLEX>::operator+=(const Matrix<COMPLEX>& other) {
    ILP_TYPE iONE = 1;
    ILP_TYPE size = this->n() * this->m();
    COMPLEX one(1, 0);

    zaxpy(&size, &one, other.data(), &iONE, this->data(), &iONE);
...
Matrix<double>& Matrix<double>::operator-=(const Matrix<double>& other) {
    ...
    daxpy(&size, &minus_one, other.data(), &iONE, this->data(), &iONE);
...
Matrix<COMPLEX>& Matrix<COMPLEX>::operator-=(const Matrix<COMPLEX>& other) {
    ...
    zaxpy(&size, &minus_one, other.data(), &iONE, this->data(), &iONE);
...
void QME_OPT_Runge_Kutt_4(const std::vector<double>& x,
    ...
    for (int i = 0; i < probs.n(); i++) {
        probs[i][0] = std::abs(y[i][i]);
    }
    Matrix<COMPLEX> new_y(y0.matrix_style(), dim, dim);
    ...
    for (size_t i = 0; i < len - 1; i++) {
        ...
        for (int j = 0; j < probs.n(); j++) {
            probs[j][i + 1] = std::abs(new_y[j][j]);
        }
