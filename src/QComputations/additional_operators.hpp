...
// vector * num
template <typename T> std::vector<T> operator*(const std::vector<T> &v, T num) {
...
// num * vector
template <typename T> std::vector<T> operator*(T num, const std::vector<T> &v) { return v * num; }
...
template <> struct std::hash<std::vector<COMPLEX>> {
...
template <> struct std::hash<std::map<int, std::vector<int>>> {
