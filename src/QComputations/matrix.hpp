...
template<typename T> class Matrix {
    public:
        ...
        explicit Matrix(MATRIX_STYLE matrix_style, const std::vector<T>& mass, size_t n, size_t m): n_(n), m_(m), mass_(mass), matrix_style_(matrix_style) { std::cout << "HERE!\n";}
        explicit Matrix(const std::vector<T>& mass): n_(mass.size()), m_(1), mass_(mass), matrix_style_(C_STYLE) {}
...
template<typename T>
Matrix<T>::Matrix(MATRIX_STYLE matrix_style, size_t n, size_t m, std::function<T(size_t, size_t)> func): matrix_style_(matrix_style), n_(n), m_(m), mass_(n_ * m_) {
    for (size_t i = 0; i < n_; i++) {
        for (size_t j = 0; j < m_; j++) {
...
Matrix<T> Matrix<T>::transpose() const {
    ...
    for (size_t i = 0; i < m_; i++) {
        for (size_t j = 0; j < n_; j++) {
            res.elem(i, j) = mass_[this->get_index(j, i)];
        }
    }
...
Matrix<T> Matrix<T>::hermit() const {
    ...
    for (size_t i = 0; i < m_; i++) {
        for (size_t j = 0; j < n_; j++) {
            res.elem(i, j) = std::conj(mass_[this->get_index(j, i)]);
        }
    }
...
void QME_OPT_Runge_Kutt_4(const std::vector<double>& x,
                                        const Matrix<COMPLEX>& y0,
                                        std::function<void(double, const Matrix<COMPLEX>&, Matrix<COMPLEX>&)> f,
                                        Matrix<double>& probs);

void QME_OPT_Runge_Kutt_2(const std::vector<double>& x,
                                        const Matrix<COMPLEX>& y0,
                                        std::function<void(double, const Matrix<COMPLEX>&, Matrix<COMPLEX>&)> f,
                                        Matrix<double>& probs);
