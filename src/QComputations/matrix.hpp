...
template <typename T> T conj(T a) {
    if (typeid(T) == typeid(std::complex<int>) or typeid(T) == typeid(std::complex<short>) or
        typeid(T) == typeid(std::complex<double>) or typeid(T) == typeid(std::complex<long>) or
        typeid(T) == typeid(std::complex<double>) or typeid(T) == typeid(std::complex<long double>)) {
        return std::conj(a);
    } else {
        return a;
    }
...
template <typename T> class Matrix {
  public:
    ...
    explicit Matrix(MATRIX_STYLE matrix_style, size_t n, size_t m)
        : matrix_style_(matrix_style), n_(n), m_(m), mass_(n_ * m_) {}
    ...
    template <typename V> Matrix(const Matrix<V> &A) : n_(A.n()), m_(A.m()) {
    ...
    // void set_multiply_mode(int multiply_mode) { MULTIPLY_MODE =
    // multiply_mode;
    // }
...
template <typename T>
Matrix<T>::Matrix(MATRIX_STYLE matrix_style, size_t n, size_t m, std::function<COMPLEX(size_t, size_t)> func)
    : matrix_style_(matrix_style), n_(n), m_(m), mass_(n_ * m_) {
    for (size_t i = 0; i < n_; i++) {
        for (size_t j = 0; j < m_; j++) {
...
template <typename T> void Matrix<T>::to_fortran_style() {
...
template <typename T> void Matrix<T>::to_c_style() {
...
template <typename T> Matrix<T> Matrix<T>::submatrix(size_t n, size_t m, size_t row_index, size_t col_index) const {
...
template <typename T> void Matrix<T>::add_rows(size_t n) {
    if (matrix_style_) {
        n_ += n;
...
template <typename T> void Matrix<T>::add_cols(size_t m) {
    if (matrix_style_) {
        for (size_t i = 0; i < n_; i++) {
...
template <typename T> void Matrix<T>::expand(size_t n) {
    this->add_rows(n);
    this->add_cols(n);
}

// MAKE FOR FORTRAN
template <typename T> void Matrix<T>::remove_rows(size_t n) {
    n_ -= n;
    mass_.resize(n_ * m_);
}

// MAKE FOR FORTRAN
template <typename T> void Matrix<T>::remove_cols(size_t m) {
    m_ -= m;
    for (size_t i = 0; i < n_; i++) {
...
template <typename T> void Matrix<T>::reduce(size_t n) {
    this->remove_rows(n);
    this->remove_cols(n);
}

template <typename T> Matrix<T>::Matrix(const std::vector<std::vector<T>> &A, MATRIX_STYLE matrix_style) {
...
template <typename T> void Matrix<T>::modify_row(size_t index, const std::vector<T> &v) {
    for (size_t j = 0; j < m_; j++) {
        mass_[this->get_index(index, j)] = v[j];
    }
}

template <typename T> void Matrix<T>::modify_col(size_t index, const std::vector<T> &v) {
    for (size_t i = 0; i < n_; i++) {
...
template <typename T> Matrix<T> Matrix<T>::operator-(const T &num) const {
...
template <typename T> std::vector<T> Matrix<T>::operator*(const std::vector<T> &v) const {
    assert(m_ == v.size());
...
template <typename T> std::vector<T> operator*(const std::vector<T> &v, const Matrix<T> &A) {
    assert(A.n_ == v.size());
    std::vector<T> res(v.size(), T(0));
...
template <typename T> std::vector<T> Matrix<T>::row(size_t index) const {
...
template <typename T> std::vector<T> Matrix<T>::col(size_t index) const {
...
template <typename T> Matrix<T> Matrix<T>::hermit() const {
    assert(typeid(T) == typeid(std::complex<double>) or typeid(T) == typeid(std::complex<int>) or
           typeid(T) == typeid(std::complex<short>) or typeid(T) == typeid(std::complex<long double>) or
...
template <typename T> void Matrix<T>::show(size_t width) const {
    for (size_t i = 0; i < n_; i++) {
        for (size_t j = 0; j < m_; j++) {
...
template <> Matrix<COMPLEX> &Matrix<COMPLEX>::operator*=(const Matrix<COMPLEX> &A);

template <> Matrix<double> &Matrix<double>::operator*=(const Matrix<double> &A);

/*-------------------------- FUNCTIONS -------------------- */

template <typename T> Matrix<T> E_Matrix(size_t n) {
...
template <typename T>
Matrix<T> exp(const Matrix<T> &A, double t, COMPLEX arg = COMPLEX(1, 0),
              int EXP_ACCURACY = QConfig::instance().exp_accuracy()) {
...
template <typename T>
void optimized_multiply(const Matrix<T> &A, const Matrix<T> &B, Matrix<T> &C, T alpha, T betta, char trans_A = 'N',
                        char trans_B = 'N');
...
OPT_Runge_Kutt_4(const std::vector<double> &x, const Matrix<COMPLEX> &y0,
                 std::function<void(double, const Matrix<COMPLEX> &, Matrix<COMPLEX> &)> f);

std::vector<Matrix<COMPLEX>>
OPT_Runge_Kutt_2(const std::vector<double> &x, const Matrix<COMPLEX> &y0,
                 std::function<void(double, const Matrix<COMPLEX> &, Matrix<COMPLEX> &)> f);

/* ------------------------- DON'T TOUCH -------------------- */

template <> lapack_complex_double *Matrix<COMPLEX>::to_upper_lapack() const;

template <> lapack_complex_double *Matrix<COMPLEX>::to_lapack() const;
