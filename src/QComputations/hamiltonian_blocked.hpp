...
template <typename StateType> class BLOCKED_H_by_Operator : public BLOCKED_Hamiltonian {
  public:
...
BLOCKED_H_by_Operator<StateType>::BLOCKED_H_by_Operator(
    ILP_TYPE ctxt, const State<StateType> &init_state, const Operator<StateType> &H_op,
    const std::vector<std::pair<double, Operator<StateType>>> &decoherence) {
...
template <typename StateType> class BLOCKED_H_by_Scalar_Product : public BLOCKED_Hamiltonian {
  public:
...
template <typename StateType>
BLOCKED_H_by_Scalar_Product<StateType>::BLOCKED_H_by_Scalar_Product(
    ILP_TYPE ctxt, const State<StateType> &init_state,
    const std::function<COMPLEX(const StateType &i, const StateType &j)> &func, BasisType<StateType> basis) {
    if (basis.empty()) {
