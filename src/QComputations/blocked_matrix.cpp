...
extern "C" {
// void pztranu(ILP_TYPE*, ILP_TYPE*, const COMPLEX*, const COMPLEX*, ILP_TYPE*,
// ILP_TYPE*,
//              ILP_TYPE*, COMPLEX*, COMPLEX*, ILP_TYPE*, ILP_TYPE*, ILP_TYPE*);
// void pdtran(ILP_TYPE*, ILP_TYPE*, const double*, const double*, ILP_TYPE*,
// ILP_TYPE*,
...
template <> double BLOCKED_Matrix<double>::get(size_t i, size_t j) const {
    return mpi::pdelget(local_matrix_, i, j, this->desc());
}

template <> COMPLEX BLOCKED_Matrix<COMPLEX>::get(size_t i, size_t j) const {
    return mpi::pzelget(local_matrix_, i, j, this->desc());
}

template <> void BLOCKED_Matrix<double>::set(size_t i, size_t j, double num) {
    mpi::pdelset(local_matrix_, i, j, num, this->desc());
}

template <> void BLOCKED_Matrix<COMPLEX>::set(size_t i, size_t j, COMPLEX num) {
    mpi::pzelset(local_matrix_, i, j, num, this->desc());
}

template <> BLOCKED_Matrix<double> BLOCKED_Matrix<double>::operator-(const BLOCKED_Matrix<double> &B) const {
...
template <> BLOCKED_Matrix<COMPLEX> BLOCKED_Matrix<COMPLEX>::operator-(const BLOCKED_Matrix<COMPLEX> &B) const {
    assert(this->matrix_type_ == GE and B.matrix_type_ == GE);
    assert(this->n_ == B.n_ and this->m_ == B.m_);
...
template <> BLOCKED_Matrix<double> BLOCKED_Matrix<double>::hermit() const {
    if (matrix_type_ == GE) {
...
template <> BLOCKED_Matrix<COMPLEX> BLOCKED_Matrix<COMPLEX>::hermit() const {
    if (matrix_type_ == GE) {
...
template <> void BLOCKED_Matrix<double>::write_to_csv_file(const std::string &filename) const {
...
template <> void BLOCKED_Matrix<COMPLEX>::write_to_csv_file(const std::string &filename) const {
    const char *charname = filename.c_str();
    size_t max_number_size = QConfig::instance().csv_max_number_size();
    size_t num_accuracy = QConfig::instance().csv_num_accuracy();
