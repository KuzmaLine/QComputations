...
template <typename T> void print_distributed_matrix(const Matrix<T> &A, const std::string &matrix_name, ILP_TYPE ctxt) {
    int myid, numproc;
...
// Распределяет по Blacs решётке блоки матрицы. Реализовано только для
// Matrix<double> и Matrix<std::complex<double>>
template <typename T>
Matrix<T> scatter_blacs_matrix(const Matrix<T> &A, ILP_TYPE &N, ILP_TYPE &M, ILP_TYPE &NB, ILP_TYPE &MB,
                               ILP_TYPE &nrows, ILP_TYPE &ncols, ILP_TYPE &ctxt, ILP_TYPE root_id,
                               MPI_Comm comm = MPI_COMM_WORLD);

template <>
Matrix<double> scatter_blacs_matrix<double>(const Matrix<double> &A, ILP_TYPE &N, ILP_TYPE &M, ILP_TYPE &NB,
                                            ILP_TYPE &MB, ILP_TYPE &nrows, ILP_TYPE &ncols, ILP_TYPE &ctxt,
                                            ILP_TYPE root_id, MPI_Comm comm);
template <>
Matrix<COMPLEX> scatter_blacs_matrix<COMPLEX>(const Matrix<COMPLEX> &A, ILP_TYPE &N, ILP_TYPE &M, ILP_TYPE &NB,
                                              ILP_TYPE &MB, ILP_TYPE &nrows, ILP_TYPE &ncols, ILP_TYPE &ctxt,
                                              ILP_TYPE root_id, MPI_Comm comm);

// Распределяет обратно.
template <typename T>
void gather_blacs_matrix(const Matrix<T> &localC, Matrix<T> &C, ILP_TYPE N, ILP_TYPE M, ILP_TYPE NB, ILP_TYPE MB,
                         ILP_TYPE nrows, ILP_TYPE ncols, ILP_TYPE ctxt, ILP_TYPE root_id);

template <>
void gather_blacs_matrix<double>(const Matrix<double> &localC, Matrix<double> &C, ILP_TYPE N, ILP_TYPE M, ILP_TYPE NB,
                                 ILP_TYPE MB, ILP_TYPE nrows, ILP_TYPE ncols, ILP_TYPE ctxt, ILP_TYPE root_id);
template <>
...
template <typename T>
std::vector<T> scatter_blacs_vector(const std::vector<T> &v, ILP_TYPE &N, ILP_TYPE &NB, ILP_TYPE &nrows, ILP_TYPE &ctxt,
                                    ILP_TYPE root_id, MPI_Comm comm = MPI_COMM_WORLD);

template <>
std::vector<double> scatter_blacs_vector<double>(const std::vector<double> &v, ILP_TYPE &N, ILP_TYPE &NB,
                                                 ILP_TYPE &nrows, ILP_TYPE &ctxt, ILP_TYPE root_id, MPI_Comm comm);

template <>
std::vector<COMPLEX> scatter_blacs_vector<COMPLEX>(const std::vector<COMPLEX> &v, ILP_TYPE &N, ILP_TYPE &NB,
                                                   ILP_TYPE &nrows, ILP_TYPE &ctxt, ILP_TYPE root_id, MPI_Comm comm);

template <typename T>
void gather_blacs_vector(const std::vector<T> &local_y, std::vector<T> &y, ILP_TYPE N, ILP_TYPE NB, ILP_TYPE nrows,
                         ILP_TYPE ctxt, ILP_TYPE root_id);

template <>
void gather_blacs_vector<double>(const std::vector<double> &local_y, std::vector<double> &y, ILP_TYPE N, ILP_TYPE NB,
                                 ILP_TYPE nrows, ILP_TYPE ctxt, ILP_TYPE root_id);

template <>
void gather_blacs_vector<COMPLEX>(const std::vector<COMPLEX> &local_y, std::vector<COMPLEX> &y, ILP_TYPE N, ILP_TYPE NB,
                                  ILP_TYPE nrows, ILP_TYPE ctxt, ILP_TYPE root_id);
...
std::vector<ILP_TYPE> descinit(ILP_TYPE n, ILP_TYPE m, ILP_TYPE NB, ILP_TYPE MB, ILP_TYPE rsrc, ILP_TYPE csrc,
                               ILP_TYPE ctxt, ILP_TYPE LLD, ILP_TYPE info);

// only for n x n matrix
template <typename T> std::vector<T> get_diagonal_elements(Matrix<T> &localA, const std::vector<ILP_TYPE> &desca);
...
void parallel_dgeadd(const Matrix<double> &A, Matrix<double> &C, const std::vector<ILP_TYPE> &desca,
                     const std::vector<ILP_TYPE> &descc, double alpha = 1.0, double betta = 1.0, char op_A = 'N');

// N - not trans. T - trans. C - hermit
void parallel_zgeadd(const Matrix<COMPLEX> &A, Matrix<COMPLEX> &C, const std::vector<ILP_TYPE> &desca,
                     const std::vector<ILP_TYPE> &descc, COMPLEX alpha = COMPLEX(1, 0), COMPLEX betta = COMPLEX(1, 0),
                     char op_A = 'N');
...
void parallel_dgemm(const Matrix<double> &A, const Matrix<double> &B, Matrix<double> &C,
                    ...
                    const std::vector<ILP_TYPE> &descc, double alpha = 1, double betta = 0, char op_A = 'N',
                    char op_B = 'N');

// N - not trans. T - trans. C - hermit
void parallel_zgemm(const Matrix<COMPLEX> &A, const Matrix<COMPLEX> &B, Matrix<COMPLEX> &C,
                    const std::vector<ILP_TYPE> &desca, const std::vector<ILP_TYPE> &descb,
                    const std::vector<ILP_TYPE> &descc, COMPLEX alpha = COMPLEX(1, 0), COMPLEX betta = COMPLEX(0, 0),
...
void parallel_dgemv(const Matrix<double> &A, const std::vector<double> &x, std::vector<double> &y,
                    ...
                    const std::vector<ILP_TYPE> &descy, char op_A = 'N');
void parallel_zgemv(const Matrix<COMPLEX> &A, const std::vector<COMPLEX> &x, std::vector<COMPLEX> &y,
                    const std::vector<ILP_TYPE> &desca, const std::vector<ILP_TYPE> &descx,
                    const std::vector<ILP_TYPE> &descy, char op_A = 'N');

// x*y (without conjugate)
COMPLEX parallel_zdotu(const std::vector<COMPLEX> &x, const std::vector<COMPLEX> &y, const std::vector<ILP_TYPE> &descx,
                       ILP_TYPE incx, const std::vector<ILP_TYPE> &descy, ILP_TYPE incy);

// <x|y>
COMPLEX parallel_zdotc(const std::vector<COMPLEX> &x, const std::vector<COMPLEX> &y, const std::vector<ILP_TYPE> &descx,
                       ILP_TYPE incx, const std::vector<ILP_TYPE> &descy, ILP_TYPE incy);
double parallel_ddot(const std::vector<double> &x, const std::vector<double> &y, const std::vector<ILP_TYPE> &descx,
                     ILP_TYPE incx, const std::vector<ILP_TYPE> &descy, ILP_TYPE incy);
...
void parallel_daxpy(const std::vector<double> &x, std::vector<double> &y, const std::vector<ILP_TYPE> &descx,
                    ILP_TYPE incx, const std::vector<ILP_TYPE> &descy, ILP_TYPE incy, double alpha = 1.0);
void parallel_zaxpy(const std::vector<COMPLEX> &x, std::vector<COMPLEX> &y, const std::vector<ILP_TYPE> &descx,
                    ILP_TYPE incx, const std::vector<ILP_TYPE> &descy, ILP_TYPE incy, COMPLEX alpha = COMPLEX(1, 0));
