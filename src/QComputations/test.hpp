...
namespace matrix {
template <typename T> Matrix<T> create_rand_matrix(size_t n, size_t m, T a, T b) {
...
template <typename T> Matrix<T> create_rand_complex_matrix(size_t n, size_t m, T a, T b) {
    Matrix<T> A(C_STYLE, n, m);
...
namespace printing {
template <typename T> void binary_print(T num) {
    T mask = T(0);
...
#ifdef ENABLE_MPI
#ifdef ENABLE_CLUSTER
void probs_print(const BLOCKED_Matrix<double> &probs, const std::set<Basis_State> &basis,
...
void check_probs(const Matrix<double> &probs, const std::set<TCH_State> &basis, const std::vector<double> &time_vec,
                 double eps = QConfig::instance().eps()) {
    for (size_t i = 0; i < time_vec.size(); i++) {
        size_t index = 0;
        double sum = 0;
        for (const auto &b : basis) {
            // std::cout << std::setw(QConfig::instance().width()) <<
            // b.to_string() << " : ";
            sum += probs[index++][i];
            // std::cout << std::setw(QConfig::instance().width()) <<
            // probs[index][i]
        ...
        if (std::abs(double(1) - sum) >= eps) {
            std::cout << std::setw(QConfig::instance().width()) << time_vec[i] << " INCORRECT PROBS! SUM = " << sum
                      << std::endl;
        }
    }
...
void check_runge_kutt(const std::vector<V> &X, const T &y0, std::function<T(V, T)> f, std::function<T(V)> f_correct) {
    ...
    size_t index = 0;
    for (const auto &x : X) {
        std::cout << std::setw(QConfig::instance().width()) << x << " -> " << y[index++] << " | " << f_correct(x)
                  << std::endl;
    }
}

template <typename T> void check_pro_race(const Matrix<T> &A, const std::vector<T> &x, const std::vector<T> &b) {
    show_vector(A * x);
    show_vector(b);
}
...
void check_eigenvectors(std::vector<double> eigenvalues, const Matrix<T> &eigenvectors, const Matrix<T> &matrix) {
    ...
    for (int i = 0; i < eigenvalues.size(); i++) {
        // Получаем собственное значение и соответствующий ему собственный
        // вектор
