...
template <typename StateType> using BasisType = std::set<std::shared_ptr<StateType>, State_Comparator>;
...
class TCH_State : public Basis_State {
  ...
  public:
    ...
    TCH_State(const Basis_State &base)
        : Basis_State(base), x_size_(base.get_groups_count()), y_size_(1), z_size_(1),
...
template <typename StateType> class State {
  public:
    ...
    std::string to_string() const {
        ...
        size_t index = 0;
        for (auto st : state_components_) {
            res += "(" + std::to_string(state_vec_[index].real()) + " + " + std::to_string(state_vec_[index].imag()) +
                   "j)";
            index++;
