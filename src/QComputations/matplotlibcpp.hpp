...
// ####################### Отремонтированный грааль данной либы, если не хотите  #######################
// ####################### вешаться от шикарного Python API для C++ - тогда 100% #######################
// #######################               !!!   НЕ ТРОГАТЬ  !!!                   #######################
...
inline PyObject* init_3d_axis(PyObject* fig) {
    ...
    PyObject* axis = PyObject_Call(
        gca, detail::_interpreter::get().s_python_empty_tuple, detail::_interpreter::get().s_python_empty_tuple);
...
inline void figure_size(size_t w, size_t h, const size_t dpi = 100) {
    ...
    PyObject* res = PyObject_Call(
        detail::_interpreter::get().s_python_function_figure, detail::_interpreter::get().s_python_empty_tuple, kwargs);
...
inline void legend(const std::map<std::string, std::string>& keywords) {
    ...
    PyObject* res = PyObject_Call(
        detail::_interpreter::get().s_python_function_legend, detail::_interpreter::get().s_python_empty_tuple, kwargs);
    if (!res) throw std::runtime_error("Call to legend() failed.");
...
inline void xkcd() {
    ...
    res = PyObject_Call(
        detail::_interpreter::get().s_python_function_xkcd, detail::_interpreter::get().s_python_empty_tuple, kwargs);
