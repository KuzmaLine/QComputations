...
int main(int argc, char** argv) {
    ...
    BLOCKED_H_by_Operator<Hydrogen_State> H(ctxt, State<Hydrogen_State>(state), my_H, dec);
    ...
    auto probs = Evolution::quantum_master_equation(State<Hydrogen_State>(state).fit_to_basis(H.get_basis()), H, time_vec);

    //basis_to_file("basis_check.csv", H.get_basis());    
    make_probs_files(H, probs, time_vec, H.get_basis(), "test_dir");
