...
OpType H = OpType(some_op_1_1) * OpType(some_op_1_2) * ... * 
        ...
        файле state.hpp. Вкраце, по своей сути, - это мудрёный \textbf{std::set<Basis\_State>}). Так сделано по причине экономии памяти и в принципе по другому унифицировать эффективно
...
\end{mdframed}
    \section{Прототипы функций и классов}
        ...
        matrix\_style - хранения матрицы по строкам (Си стиль - C\_STYLE) или по столбцам (Фортрановский стиль - FORTRAN\_STYLE).
        Файлы matrix.*
...
\end{mdframed}
        ...
        examples лежит подобный пример - \textsf{tch_gif_example.cpp}.
...
\begin{center}{\textbf{get\_elem\_from\_basis - получить элемент из BasisType по индексу.}}\end{center}
...
T get_elem_from_basis(const BasisType<T>& st, size_t index);
    \end{minted}
\end{mdframed}    
...
\end{mdframed}
        ...
        Также важным моментом являются 2 виртуальных метода: to_string и оператор сравнения.
        \begin{enumerate}
        \item{Перегрузка to_string несёт исключительно косметический эффект, несмотря на то, что сортировка \textbf{Basis\_State} сделана с помощью собственно метода to_string.}
        ...
        Пример перегрузки методов продемонстрирован, например, в примере \textsf{realization_tch_example.cpp}.
