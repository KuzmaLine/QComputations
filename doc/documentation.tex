...
\end{mdframed}
        ...
        \begin{itemize}
            ...
            \item{BLOCKED\_Hamiltonian (матрица хранится блочно распределённо)}
        \end{itemize}
        ...
        \begin{itemize}
            \item{Hamiltonian\_by\_Operator<StateType>}
        \end{itemize}
...
\end{mdframed}
        ...
        \subsection{BLOCKED\_Hamiltonian}

        Для BLOCKED\_Hamiltonian:
        \begin{itemize}
            \item{BLOCKED\_Hamiltonian\_by\_Operator<StateType>}
        \end{itemize}
...
\end{mdframed}
        ...
        Реализовано моделирование с помощью двух уравнений. Уравнение Шрёдингера (schrodinger) через спектральное разложение и через основное квантовое уравнение (quantum\_master\_equation),
        ...
        Оба методы реализованы как для обычного гамильтониана (Hamiltonian), так и для блочного (BLOCKED\_Hamiltonian).

        quantum\_master\_equation паралеллится только с помощью блочных матриц, но, при размере подблоков матрицы (NB, MB) <= 32, вместо ускорения будет видно сильное замедление, поэтому при 
...
\end{mdframed}
    ...
    По столбцам - вектор времени time\_vec (см. главу Моделирование динамики гамильтонианов).
...
\end{mdframed}
        ...
        Данный способ более универсален и поддерживает параллельную генерацию графиков. Все результаты записываются в файлы формата CSV.
         ...
         данного скрипта находится в конфигурационном файле, путь к которому находится в переменной \$SEABORN\_CONFIG.

         Начнём с команд для создания файлов. \textbf{\$SEABORN\_PLOT работает только с файлами, созданными с помощью make\_probs\_files!}
...
// Записать гамильтониан в CSV файл. main_rank???????????????????????????????
...
// Записать вероятности в CSV файл, находящиеся в виде одноядерной матрицы на ядре main_rank
...
\end{mdframed}
        ...
        Теперь описание самого скрипта. Настройки хранятся в конфигурационном файле, хранящимся на пути \$SEABORN\_CONFIG. Пример выглядит он следующим образом:
...
\end{mdframed}
        ...
        \begin{itemize}
            \item{format - формат результата файла. Поддерживаются png, jpg и gif. Для генерации gif смотреть пример - <???????????????????>}
...
\end{mdframed}
        ...
        На текущий момент реализован ???алгоритм отбора рабочей области???. Он работает следующим образом:

        Берётся начальное состояния - каждая его базисная компонента прогоняется через оператор. Состояния добавляются в базис.
        Алгоритм работает до тех пор, пока появляются новые состояния. Реализация - файл graph.hpp
...
class State_Graph {
    ...
    private:
        std::set<StateType> basis_; // Полученный базис
};
\end{minted}
\end{mdframed}
        ...
        \begin{itemize}
            \item{Тавис-Каммингс-Хаббард. (H\_TCH, BLOCKED\_H\_TCH)}
        \end{itemize}

        \subsection{Вспомогательные функции для создания собственных операторов}
        Функции для удобства перехода от базисного состояния StateType к суперпозиции State<StateType>.

        \begin{center}
            \textbf{set\_qudit - установка значения кудиту}
        \end{center}

        Устанавливает значению кудиту - на выход получаете данное состояния в виде State<StateType> с единичным коэффициентом.
         В случае, если устанавливается значения >= максимальному значению кудита, то в ответ возвращается пустое состояние, то есть пустой State<StateType>.
...
\end{mdframed}
        ...
        Возвращает переданное в функцию состояние в виде State<StateType> с коэффициентом, равным значению запрашиваемого кудита.
...
\end{mdframed}
        ...
        \begin{center}
            \textbf{check???? переделать на условие - оператор проверки значения кудиту}
        \end{center}
        ...
        Данный оператор проверяет значение указанного кудита - если значение подходит под условие, тогда оператор возвращает State<StateType> с единичным коэффициентом, 
        в противном случае возвращает пустое состояние.
...
\end{mdframed}
        ...
        \subsection{TCH. Модель Тависа-Каммингса-Хаббарда}

        Модель Тависа-Каммингса-Хаббарда - это модель оптических полостей. Модель проверена экспериментально, например, на Рабиевских осцилляциях для 2 полостей.

        Формула $H_{TCH}$ следующая:

        $H_{TCH} = \sum \limits_{k = 1}^{P}(\hbar{w_{ph}}{a_k}{a^{\dagger}_k} + {\hbar}{w_{at}}{\sum\limits_{i = 1}^{m_k}(\sigma_{k,i}\sigma^{\dagger}_{k,i})}
        ...
        Начнём с реализации (файлы state.*) самого состояния TCH\_State:
        Перенести реализацию????????????????????????????????????????????????????
...
class TCH_State: public Basis_State {
    ...
    public:
        ...
        // Создать состояние с количеством атомов в каждой полости, согласно grid_config
        // Пример: Для grid_config = {3, 2} конструктор создаст состояние с 2 полостями,
    ...
    private:
        ...
        std::vector<std::vector<CavityId>> neighbours_; // Соседи для каждой полости
...
\end{mdframed}
        ...
        \begin{itemize}
            \item{Оператора энергии поля (photons\_count ~ $E_{PH}$)}
            \item{Оператора энергии атомов (atoms\_exc\_count ~ $E_{AT}$)}
            \item{Оператор возбуждения и релаксации атомов внутри полости (exc\_relax\_atoms ~ $UD$ (UP DOWN))}
            \item{Оператор перехода фотонов из полости в полость (photons\_transfer ~ $PH_{TRANS}$)}
        \end{itemize}
        ...
        Реализовывает оператор ${\hbar}{w_{at}}$
...
State<TCH_State> photons_count(const TCH_State& state) {
    ...
    for (size_t i = 0; i < state.cavities_count(); i++) {
        // В коэффициент прибавляем все кудиты, соответствующие фотонам в каждой полости
...
\end{mdframed}
        ...
        Реализовывает оператор ${\hbar}{w_{at}}{\sum\limits_{i = 1}^{m_k}(\sigma_{k,i}\sigma^{\dagger}_{k,i})}$
...
\end{mdframed}
        ...
        Описываем взаимодействие атомов с полем, то есть оператор ${g}\sum\limits_{i=1}^{m_k}({a_k}\sigma^{\dagger}_{k,i} + a^{\dagger}_k\sigma_{k,i})$
...
\end{mdframed}
    ...
    Разберём реализацию одноядерного гамильтониана. Замечу следующее: так как мы генерируем гамильтониан через операторы - H\_TCH является дочерним классом H\_by\_Operator<TCH\_State>
\begin{mdframed}[linecolor=black, topline=true, bottomline=true,
leftline=false, rightline=false]
\begin{minted}[mathescape]{c++}
