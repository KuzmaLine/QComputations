...
\end{mdframed}
    ...
    \begin{center}
        \textbf{is_main_proc - булевая функция. Выдаёт true только для ядра с рангом 0 в MPI_COMM_WORLD}
    \end{center}

    Реализация в файле mpi_functions.cpp
...
\end{mdframed}
    ...
    \begin{center}
        \textbf{get_proc_rank - Возвращает ранг ядра в MPI_COMM_WORLD}
    \end{center}
...
std::pair<Probs, std::set<Basis_State>> probs_to_cavity_probs(
                            const Probs& probs,
                            const std::set<Basis_State>& basis,
                            size_t cavity_id);

std::pair<BLOCKED_Probs, std::set<Basis_State>> probs_to_group_probs(
                            const BLOCKED_Probs& probs,
                            const std::set<Basis_State>& basis,
                            size_t group_id);

std::pair<BLOCKED_Probs, std::set<Basis_State>> probs_to_cavity_probs(
                            const BLOCKED_Probs& probs,
                            const std::set<Basis_State>& basis,
                            size_t cavity_id);
\end{minted}
...
void plot_from_files(const std::string& plotname,
                     ...
                     const std::string& python_script_path =
                          QConfig::instance().python_script_path());
...
\end{mdframed}
        ...
        \begin{center}
        \textbf{BLOCKED_H\_TCH}
        \end{center}
        
        BLOCKED\_H\_TCH аналогичен H\_TCH.
\begin{mdframed}[linecolor=black, topline=true, bottomline=true,
leftline=false, rightline=false]
\begin{minted}[mathescape]{c++}
