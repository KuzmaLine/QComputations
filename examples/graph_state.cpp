...
int main(int argc, char** argv) {
    ...
    size_t cavity_counts = 3;
    std::vector<size_t> grid_atoms;
    std::pair<double, double> gamma = std::make_pair(0.01, 1);
    std::pair<double, double> pair_zero = std::make_pair(0, 0);
    Matrix<std::pair<double, double>> waveguides_parametrs({{pair_zero, gamma, pair_zero}, {gamma, pair_zero, gamma}, {pair_zero, gamma, pair_zero}});
    ...
    state.set_leak_for_cavity(2, 0.001);
    state.set_n(1, 0);
