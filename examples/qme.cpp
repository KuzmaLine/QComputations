...
class C_State {
    public:
        explicit C_State(size_t n, const std::vector<int> atoms_states): n_(n), atoms_states_(atoms_states),
                                                                         leak_(0), gain_(0), max_N_(atoms_states.size()) {}
        ...
        void set_leak(double gamma) { leak_ = gamma; }
        double get_leak() const { return leak_; }
        void set_gain(double gamma) { gain_ = gamma; }
...
COMPLEX photon_destroy(const C_State& state_from, const C_State& state_to) {
    auto tmp_state = state_from;

    bool is_founded = false;

    tmp_state.set_n(tmp_state.n() - 1);
...
COMPLEX photon_create(const C_State& state_from, const C_State& state_to) {
    auto tmp_state = state_from;
...
std::set<C_State> generate_basis(const C_State& init_state) {
    ...
    while(!queue.empty()) {
        ...
        //a
        if (cur_state.get_leak() > eps and cur_state.n() > 0) {
        ...
        //a+
        if (cur_state.get_gain() > eps and cur_state.n() < cur_state.max_N()) {
            cur_state.set_n(cur_state.n() - 1);
...
BLOCKED_Matrix<COMPLEX> A_leak(int ctxt, const C_State& state, const std::set<C_State>& basis) {
    ...
    [&basis](size_t i, size_t j) {
        ...
        if (state_from.n() != 0) return photon_destroy(state_from, state_to);
...
BLOCKED_Matrix<COMPLEX> A_gain(int ctxt, const C_State& state, const std::set<C_State>& basis) {
    size_t dim = basis.size();
    ...
    [&basis](size_t i, size_t j) {
        ...
        if (state_from.n() < state_from.max_N()) return photon_create(state_from, state_to);
        else return COMPLEX(0);
    }};
...
BLOCKED_Matrix<COMPLEX> lindblad(const BLOCKED_Matrix<COMPLEX>& rho, const BLOCKED_Matrix<COMPLEX>& A, double gamma) {
    auto Aconj = A.hermit();
    auto AconjA = Aconj * A;
...
int main(int argc, char** argv) {
    ...
    if (gamma > eps) {
        auto A = A_leak(H.ctxt(), state, basis);
        A.show();
        lindblads.push_back(std::function<Evolution::BLOCKED_Rho(const Evolution::BLOCKED_Rho& rho)> {
            [A, gamma](const Evolution::BLOCKED_Rho& rho) {
                return lindblad(rho, A, gamma);
            }
        }
        );
    }

    gamma = state.get_gain();
    if (gamma > eps) {
        auto A = A_gain(H.ctxt(), state, basis);
        A.show();
        lindblads.push_back(std::function<Evolution::BLOCKED_Rho(const Evolution::BLOCKED_Rho& rho)> {
            [A, gamma](const Evolution::BLOCKED_Rho& rho) {
                return lindblad(rho, A, gamma);
